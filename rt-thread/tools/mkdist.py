...
def bsp_update_rtconfig(dist_dir):
    # # delete testcases in rt-thread/Kconfig
    # if not os.path.isfile(os.path.join(dist_dir, 'rt-thread/Kconfig')):
    #     return
    ...
    if os.path.isdir(dist_dir):
        projects = os.listdir(dist_dir)
        # is a project path?
        if (".config" in projects) and ("Kconfig" in projects):
            print('new bsp path {}'.format(dist_dir))
            try:
                os.system('scons --pyconfig-silent -C {0}'.format(dist_dir)) # update rtconfig.h and .config
...
def bs_update_ide_project(bsp_root, rtt_root, rttide = None):
    ...
    if rttide == None:
        ...
        item = 'eclipse --reset-project-config --project-name=' + rttide['project_name']
